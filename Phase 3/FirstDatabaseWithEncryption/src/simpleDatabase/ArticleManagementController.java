...
/**
 * Controller class for article management functionalities.
 * Handles creation, deletion, backup, restore, and searching of help articles.
 */
public class ArticleManagementController {
    ...
    public void openCreateArticleScreen(Stage window, UserAccount user) {
        ...
        createBtn.setOnAction(e -> {
            ...
            if (title.isEmpty() || authors.isEmpty() || abstractText.isEmpty() || body.isEmpty()) {
                statusLbl.setText("Fill in all fields.");
                return;
            }
            ...
            if (groupDB.containsKey(groupName)) {
                ...
                if (group.getInstructorAdmins().contains(user.getUsername())) {
                    group.addArticle(id);
    ...
    public void openManageArticlesScreen(Stage window, UserAccount user) {
        ...
        // Admins and Teachers can delete articles; they cannot view or edit articles.
        deleteBtn.setOnAction(e -> {
            ...
            if (article != null) {
                ...
                if (group != null && group.getInstructorAdmins().contains(user.getUsername())) {
    ...
    // Open screen for backing up articles
    public void openBackupArticlesScreen(Stage window, UserAccount user) {
        // Simulate backup functionality
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Backup Articles");
        alert.setHeaderText(null);
        alert.setContentText("Articles have been backed up successfully.");
        alert.showAndWait();

        displayHomeScreen(window, user);
    }

    // Open screen for restoring articles
    public void openRestoreArticlesScreen(Stage window, UserAccount user) {
        ...
        alert.setHeaderText(null);
        alert.setContentText("Articles have been restored successfully.");
        alert.showAndWait();
    ...
    public void openStudentSearchArticlesScreen(Stage window, UserAccount user) {
        ...
        ComboBox<String> groupCombo = new ComboBox<>();
        groupCombo.getItems().addAll("All");
        groupCombo.getItems().addAll(groupDB.keySet());
    ...
    // Open screen for instructors to search articles (same as student with added rights)
        ...
        searchBtn.setOnAction(e -> {
            ...
            for (HelpArticle article : articleDB.values()) {
                ...
                if (matchesLevel && matchesGroup && matchesQuery) {
                    ...
                    ArticleGroup group = groupDB.get(article.getGroupName());
                    if (group != null && group.isSpecialAccess()) {
                        // Check if the instructor has access to this group
                        if (group.getInstructorViewers().contains(user.getUsername()) ||
